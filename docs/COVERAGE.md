...
Статус покрытия источника и статус конкретной карточки — разные доказательства. Даже при
`complete_verified` для обхода источника поисковый результат остаётся `candidate`, пока карточка
не прочитана; локальная карточка становится `exact` только из полного официального снимка либо
прямого ответа официального API. Ручной импорт всегда остаётся `unverified`.
