...
**Как понять, что загружена актуальная версия?** После перезапуска клиент должен обнаружить 31 инструмент.
В их числе `fgis_batch_search_norms`, `fgis_batch_read_norms`, `fgis_norm_history` и
`fgis_import_opendata`. Если пакетные инструменты отсутствуют, обновите кеш командой выше и
перезапустите MCP-процесс.
