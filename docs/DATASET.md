...
- `snapshot_uid` (TEXT): канонический идентификатор конкретного импортированного снимка.
...
`snapshot_id` удобен для отображения даты, но не является уникальным ключом. Для ссылок между
таблицами, сравнения и повторного импорта используется `snapshot_uid`; если один и тот же
`snapshot_id` соответствует нескольким снимкам, неоднозначная ссылка отклоняется.
